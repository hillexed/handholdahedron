...
for(let faceIndex=0;faceIndex<12;faceIndex++){
    let edgesInThisFace = [];
    let dualVerticesInThisPlane = dualVerticesInThisPlane.filter((arr) => arr.includes(faceIndex))

    for(let vertex1 of verticesInThisFace){
        let vertex1 = verticesInThisFace[i];
        for(let vertex2 of verticesInThisFace){
            if (i == j){
                continue
            }
